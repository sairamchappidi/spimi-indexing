...
typedef struct {
           ...
           char *oldEnd;          /* suffix replaced */
           char *newEnd;          /* suffix replacement */
...
ReplaceEnd( char *word, RuleList *rule )
   ...
   while ( 0 != rule->id )
      ...
      if ( word <= ending )
         if ( 0 == strcmp(ending,rule->oldEnd) )
            {
            tmp_ch = *ending;
            *ending = EOS;
            if ( rule->min_root_size < WordSize(word) )
               if ( !rule->condition || (*rule->condition)(word) )
                  {
#ifndef OLD_CODE
									size_t n = strlen( rule->newEnd );
									(void) strncat( word, rule->newEnd, n );
#else /* OLD_CODE */
                  (void)strcat( word, rule->newEnd );
#endif /* OLD_CODE */
